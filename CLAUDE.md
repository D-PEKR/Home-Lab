...
make homeserver2     # Deploy alle Services auf homeserver2 (192.168.178.95)
make homeserver2-check # Dry-run des homeserver2-Playbooks (keine Änderungen)
...
| Paperless-NGX | http://homeserver2:8000  | homeserver2 (Docker Compose)       |
| TinyTeller    | http://homeserver2:3002  | homeserver2 (Docker Compose)       |
| Day Pilot     | http://homeserver2:3003  | homeserver2 (Docker Compose)       |
...
- Hardware button → `scanbd` detects → `scan_button.sh` touches trigger flag →
  ...
  `/mnt/paperless-consume` (homeserver2, SMB-Share).
- Paperless-NGX läuft auf homeserver2 und liest aus diesem Verzeichnis ein.
...
- **Scanner — first run needs homeserver2 reachable**: `_netdev,nofail,x-systemd.automount` keeps the host boot non-fatal when homeserver2 is down, but `make scanner` itself calls `ansible.posix.mount state=mounted` which actually performs the mount. Bring homeserver2 up before the first run, or skip the mount task with `--skip-tags scanner` until it is reachable.
...
- **Semaphore bootstrap — `body_format: json` + integer fields**: Ansible stringifiziert YAML-Skalare `"{{ expr | int }}"` vor der JSON-Serialisierung — Semaphore's Go-Decoder lehnt das mit HTTP 400 (leerem Body) ab. Fix: Jinja2-Dict-Literal in `>-`-Block verwenden (`{{ {'key': val | int} | to_json }}`) statt YAML-Dict mit Template-Skalaren und `body_format: raw`. `inventory.yml` und `template.yml` verwenden dieses Muster bereits; `key.yml` und `repository.yml` wurden nachgezogen.
